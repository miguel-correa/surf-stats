...
func (s *KSFScraper) FetchMapCompletionsFromPlayerRecord(steamID string, mapName string) (int, error) {
	...
	if err := json.Unmarshal(body, &response); err != nil {
		return -1, fmt.Errorf("failed to parse map records JSON: %w", err)
	}

	for _, zone := range response.Zones {
		if zone.ZoneID == 0 {
			return zone.TotalRanks, nil
		}
	}

	return -1, fmt.Errorf("failed to find completions for given map")
}
